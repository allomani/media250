...
$member_table_tofind =array('mobile_members');
...
function member_verify_password($userid,$pwd){
    
$qr=db_query("select ".members_fields_replace('password').",salt from ".members_table_replace('mobile_members')." where ".members_fields_replace('id')."='$userid'",MEMBER_SQL);

if(db_num($qr)){
    $data = db_fetch($qr);

 if($data[members_fields_replace('password')]==md5(md5($pwd).$data['salt'])){
 return true;
 }else{
 return false;
 }   
...
if(trim($user_email)){
 ...
 if(db_num($qr)){
     ...
     db_query("delete from mobile_confirmations where type='rest_pwd' and cat='$data[id]'");  
     db_query("insert into mobile_confirmations (type,cat,code) values('rest_pwd','$data[userid]','$active_code')");
 ...
 $data_member = db_qr_fetch("select username,email from user where userid='$data[cat]'",MEMBER_SQL);
 ...
 db_query("delete from mobile_confirmations where type='rest_pwd' and code='".db_clean_string($code)."'");    
