...
if(trim($user_email)){
 ...
 if(db_num($qr)){
     ...
     db_query("delete from songs_confirmations where type='rest_pwd' and cat='$data[id]'",MEMBER_SQL);
     db_query("insert into songs_confirmations (type,cat,code) values('rest_pwd','$data[id]','$active_code')",MEMBER_SQL);
 ...
 //------------ Rest pwd Process ----------------
 if($action=="rest_pwd"){
 $qr = db_query("select * from songs_confirmations where code='".db_clean_string($code)."'",MEMBER_SQL);
 if(db_num($qr)){
 $data =db_fetch($qr);
 $new_pwd = rand_string();
 ...
 db_query("delete from songs_confirmations where type='rest_pwd' and code='".db_clean_string($code)."'",MEMBER_SQL);
