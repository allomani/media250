...
function connector_member_pwd($userid,$pwd,$op){
    ...
    if($op=="update"){
        $pwdz = md5($pwd);
    db_query("update mobile_members set password='".$pwdz."' where id='".intval($userid)."'");
    }
}
...
$qr=db_query("select password from mobile_members where id='$userid'");
if(db_num($qr)){
    $data = db_fetch($qr);
...
if(trim($user_email)){
 $qr=db_query("select * from mobile_members where email='".db_clean_string($user_email)."'");
 if(db_num($qr)){
     $data = db_fetch($qr);
     ...
     db_query("insert into mobile_confirmations (type,cat,code) values('rest_pwd','$data[id]','$active_code')");
 ...
 $data_member = db_qr_fetch("select username,email from mobile_members where id='$data[cat]'");
