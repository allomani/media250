...
include  "members_connectors/".$members_connector['connector_file'];
}else{
include  "members_connectors/default.php";
}
...
function check_member_login(){
 ...
 $member_data['password'] = get_cookie('member_data_password');
   ...
   $qr = db_query("select * from ".members_table_replace("mobile_members")." where ".members_fields_replace("id")."='$member_data[id]'",MEMBER_SQL);
         ...
         if(db_num($qr)){
           ...
           if($data[members_fields_replace('password')] == $member_data['password']){
            ...
            if(in_array($data[members_fields_replace('usr_group')],$members_connector['allowed_login_groups'])){
               ...
               db_query("update ".members_table_replace('mobile_members')." set ".members_fields_replace('last_login')."='".connector_get_date(date("Y-m-d H:i:s"),'member_last_login')."' where ".members_fields_replace('id')."='".$member_data['id']."'",MEMBER_SQL);
...
if($action=="edit"){
    $dtsx  = db_qr_fetch("select value from mobile_members_fields where member='$memberid' and cat='$data[id]'");
...
$dtsx  = db_qr_fetch("select value from mobile_members_fields where member='$memberid' and cat='$cat'");
return "$dtsx[value]";
}
...
function snd_email_activation_msg($id){
  ...
  $active_code = md5(rand(0,999).time().$data[members_fields_replace('email')].rand().$id) ;
   ...
   db_query("delete from mobile_confirmations where type='validate_email' and cat='".$data[members_fields_replace('id')]."'");
     db_query("insert into mobile_confirmations (type,cat,code) values('validate_email','".$data[members_fields_replace('id')]."','$active_code')");
