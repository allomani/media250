...
if(!check_login_cookies()){die("<center> Access Denied </center>");}
if_admin("members");
...
if($send_to=="all"){
       $qr = db_query("select id,username,email from mobile_members where usr_group > 0 order by id limit $start,$perpage");
       }else{
        $qr = db_query("select id,username,email from mobile_members where username='".db_clean_string($username)."'");
        }


         if(db_num($qr)){

             $count_dt = db_qr_fetch("select count(id) as count from mobile_members where  usr_group > 0 order by id");
        ...
        db_query("insert into mobile_msgs (user,sender,title,content,date) values('$data[id]','".db_clean_string($from_name)."','".db_clean_string($from_subject)."','".db_clean_string($from_msg)."',now())");
