...
$qr = db_query("select * from ".members_table_replace("mobile_members")." where ".members_fields_replace("username")."='".db_clean_string($username)."'",MEMBER_SQL);
...
if(db_num($qr)){
   ...
   $data = db_fetch($qr);
    ...
    if(member_verify_password($data[members_fields_replace('id')],$password)){
